...
fn test_entity_parent_is_transferred_from_server() {
    TestRun::default().run(
        ...
        |_| {},
        |env: &mut TestEnv| {
            let e1 = env.server.world_mut().spawn(SyncMark {}).id();
            let e2 = env.server.world_mut().spawn(SyncMark {}).id();

            env.update(3);

            env.server.world_mut().entity_mut(e1).add_child(e2);
...
fn test_entity_parent_is_transferred_from_client() {
    TestRun::default().run(
        ...
        |_| {},
        |env: &mut TestEnv| {
            let e_id1 = env.clients[0].world_mut().spawn(SyncMark {}).id();
            let e_id2 = env.clients[0].world_mut().spawn(SyncMark {}).id();
